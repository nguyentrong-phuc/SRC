...
void setup()
{
	...
	DroneController.setupDroneSystem(&droneData);
	//Khoi dong RF
	...
	//Khoi dong doc EBIMU
	DroneEBI_data.EBISetOffsetValues(&(droneData.ebimu_data), -1 ,0.72, -176);
	...
	droneData.zLidar1DErr = 54; //khoang cach ban dau cua lidar
	//Khoi dong bien HMC
	DroneHMC.HMCSetup();
	...
	droneData.P_basic = Pmin;
	droneData.autoTuneState = false;
	droneData.gpsMode = 0;
...
void loop()
{
	...
	DroneController.MPUDataProcessing(&droneData);
	//Doc du lieu lidar1D
	DroneController.Lidar1D_read(&(droneData.zLidar1D));
	DroneController.AltitudeDataProcessing(&droneData);
	//Doc du lieu HMC
	...
	//Serial.print(droneData.zLidar1D - droneData.zLidar1DErr); Serial.print(", ");
  //Serial.print(droneData.awz); Serial.print(", ");
	Serial.print(droneData.z); Serial.print(", ");
...
//  Serial.println(droneData.ebimu_data.pitch); //Serial.print(", ");
 ...
 //Serial.print(droneData.mpu_data.accz); Serial.print(", ");
	...
	Serial.print(droneData.zd); Serial.print(", ");
  Serial.println(droneData.P_basic*0.01); 
