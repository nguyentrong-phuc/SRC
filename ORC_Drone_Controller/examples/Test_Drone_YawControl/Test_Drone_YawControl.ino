...
void setup()
{
	...
	//Khoi dong doc EBIMU
	DroneEBI_data.EBISetOffsetValues(&(droneData.ebimu_data), 0 , 0, -176);
...
void loop()
{
  ...
  Serial.println(droneData.yaw_d); 
//  Serial.println(droneData.mpu_data.gz);// Serial.print(", ");
  
		

}

void setupYawControl(orcdrone_data *drone_in){
	//Set cac tham so dk khac bang 0
	drone_in->pitch_kp = 5; 	drone_in->pitch_ki = 0; 	drone_in->pitch_kd = 1; drone_in->pitch_ei = 0;
	drone_in->roll_kp = 5; 		drone_in->roll_ki = 0; 		drone_in->roll_kd = 2; 	drone_in->roll_ei = 0;
	drone_in->yaw_kp = 15; 		drone_in->yaw_ki = 1; 		drone_in->yaw_kd = 4.5; 	drone_in->yaw_ei = 0;
